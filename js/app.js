...
async function marcarComoTengo(id) {
  ...
  // Pedir año de adquisición
  const anioAdquisicion = prompt('¿En qué año adquiriste esta pieza?', new Date().getFullYear());
  if (!anioAdquisicion) return;

  const nuevaPieza = {
    id: Date.now().toString(),
    catalogo_id: id,
    anio: anioAdquisicion,
    grado: 'BU',
    cantidad: '1',
    precio_compra: '0'
  };
